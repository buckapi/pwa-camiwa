...
this.getProducts().subscribe(
   ...
   getProducts(): Observable<any[]> {
    ...
    return this.getAllPages(this.urlPrev).pipe(
      tap(products => {
        ...
        products.forEach(product => categories.add(product.cat)); // Asumiendo que 'cat' es la propiedad que contiene la categoría de cada producto
  ...
  private getAllPages(url: string, products: any[] = []): Observable<any[]> {
    return this.http.get<any>(url).pipe(
      switchMap((response: any) => {
        ...
        if (response.page < response.totalPages) {
          ...
          return this.getAllPages(nextPageUrl, products);
        } else {
          ...
          return of(organizedProducts);
        }
      })
    );
  }
  
  private organizeProductsByCategory(products: any[]): any[] {
    // Crear un objeto para almacenar productos organizados por categoría
    const organizedByCategory: { [key: string]: any[] } = {};
  
    // Organizar productos por categoría
    products.forEach(product => {
      const category = product.cat; // Reemplaza 'category' con la propiedad real que almacena la categoría en tu objeto de producto
      if (!organizedByCategory[category]) {
        organizedByCategory[category] = [];
      }
      organizedByCategory[category].push(product);
      
    });
  
    // Convertir el objeto organizado nuevamente en un array
    const organizedProducts = Object.values(organizedByCategory).flat();
